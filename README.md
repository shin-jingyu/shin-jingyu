...
기능 요청을 바로 구현하기보다 기준이 되는 데이터와 시스템별 역할을 먼저 확인합니다. 연동에 제약이 있는 경우에는 단순히 어렵다고 결론 내리지 않고, 필요한 API 규격과 식별값, 통신 방향, 인프라 지원 사항을 정리해 구현 가능한 범위를 조율합니다.

---

## Experience Highlights

* 망 분리 환경의 Android–DMZ–Core 다계층 API 연동 구조 설계
* 출입 게이트 이벤트 기반 사용자 출입 상태 판별 및 MDM 정책 제어
* PostgreSQL Replication 환경 구축 및 운영
* 시스템 간 이벤트 규격, 사용자 식별값, 통신 방향 조율
* 멀티모듈, 인증 서버, API Gateway, 메시징 구조 학습 및 구현
* JWT, OAuth 2.0, Redis를 활용한 팀 프로젝트 개발
...
출입 시스템과 MDM은 관리하는 데이터와 운영 환경이 달랐기 때문에 API만 추가해서는 기능을 완성하기 어려웠습니다.

출입 상태를 판별하기 위해 필요한 이벤트 규격과 사용자 식별값을 확인하고, 각 시스템이 처리할 데이터와 통신 방향을 정리했습니다. 서버나 네트워크 영역의 지원이 필요한 경우에는 구현에 필요한 조건을 구체화해 관련 담당자와 협의했습니다.
...
아이디어 단계의 서비스를 실제 개발 가능한 기능으로 나누고, 도메인 모델과 MVP 범위를 팀원들과 함께 구체화했습니다.
...
* 멀티모듈 구조를 적용한 도메인 및 의존 관계 분리
* JPA와 QueryDSL을 활용한 데이터 저장·조회 기능 구현
...
기능 구현 결과뿐 아니라 구현 배경과 변경 범위를 Pull Request에 기록해, 팀원이 코드의 맥락을 함께 이해할 수 있도록 개발하고 있습니다.
...
Spring과 JPA를 기반으로 백엔드 기술을 직접 구현하고, 기존 방식과의 차이 및 실제 적용 방법을 토론하는 스터디입니다.
...
`Java` `Spring Boot` `Spring Security` `JPA` `MyBatis` `QueryDSL`
...
`REST API` `gRPC` `GraphQL` `RabbitMQ`
